...
Vue.component('m-image-file', {
    name: "mImageFile",
    props: {name:String, id:String, form:String},
    data: function() {
        return {
          showButton:false,
    ...
    template: '<q-card style="width:100px" > <q-card-section>' +
                '<div :style="{ \'background-image\': `url(${imageData})` }"> ' +
                  '<input :name="name" :id="id" :form="form" hidden ref="fileInput" type="file" @input="onSelectFile" accept=".jpg, image/*"/> '+
                '</div> '+
                '<q-img :src="imageData"  style="cursor: pointer" @click="choosePicture" @mouseover="showButton=true" @mouseleave="showButton=false"><q-tooltip>Click to choose a picture</q-tooltip></q-img> '+
                ' <q-btn round color="grey-3" icon="edit" size="8px" class="absolute" style="bottom:0px;left:0px;" v-show="showButton"/>' +
                ' <q-btn round color="grey-3" icon="delete" size="8px" class="absolute" style="bottom:0px;right:0px;" v-show="showButton"/>' +
