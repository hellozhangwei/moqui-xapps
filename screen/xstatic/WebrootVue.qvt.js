...
Vue.component('m-dialog', {
    ...
    template:
    '<q-dialog v-bind:value="value" v-on:input="$emit(\'input\', $event)" :id="id" @show="onShow" @hide="onHide" :maximized="$q.platform.is.mobile" no-backdrop-dismiss>' +
...
Vue.component('m-dynamic-dialog', {
    ...
    props: { id:{type:String}, url:{type:String,required:true}, color:String, buttonText:String, buttonClass:String, title:String, width:{type:String},
        openDialog:{type:Boolean,'default':false}, dynamicParams:{type:Object,'default':null}, buttonFlat:{type:Boolean} },
    data: function() { return { curComponent:moqui.EmptyComponent, curUrl:"", isShown:false} },
    template:
    '<span>' +
        '<q-btn unelevated dense size="sm" no-caps icon="open_in_new" :flat="buttonFlat" :label="buttonText" :color="color" :class="buttonClass" @click="isShown = true"></q-btn>' +
    ...
    methods: {
        ...
        removePicture: function(){
          if (confirm("Confirm to delete?")) {
            //todo call delete on server side
            this.imageData="";
          }
        }
    },
...
Vue.component('m-date-time', {
    ...
    '<q-input dense stack-label :label="label" v-bind:value="value" v-on:input="$emit(\'input\', $event)" :rules="rules"' +
        ...
        '<template v-slot:append v-if="type==\'time\' || type==\'date-time\' || !type">' +
            '<q-icon name="access_time" class="cursor-pointer" style="font-size: 12px">' +
...
Vue.component('m-date-period', {
    ...
    '<div v-if="fromThruMode" class="row">' +
        '<m-date-time :name="name+\'_from\'" :id="id+\'_from\'" :label="label+\' From\'" :form="form" :type="fromThruType"' +
            ' v-model="fields[name+\'_from\']" :bg-color="fieldChanged(name+\'_from\')?\'blue-1\':inputBgColor"></m-date-time>' +
        ...
        '<m-date-time :name="name+\'_thru\'" :id="id+\'_thru\'" :label="label+\' Thru\'" :form="form" :type="fromThruType"' +
            ' v-model="fields[name+\'_thru\']" :bg-color="fieldChanged(name+\'_thru\')?\'blue-1\':inputBgColor">' +
        ...
        '</m-date-time>' +
    '</div>' +
    '<div v-else class="row"><q-input dense stack-label :label="label" v-model="fields[name+\'_pdate\']"' +
            ' mask="####-##-##" fill-mask :id="id" :name="name+\'_pdate\'" :form="form" style="max-width:max-content;"' +
            ' :bg-color="fieldChanged(name+\'_pdate\')?\'blue-1\':inputBgColor">' +
        '<q-tooltip v-if="tooltip">{{tooltip}}</q-tooltip>' +
        '<template v-slot:before>' +
            '<q-select class="q-pr-xs" dense options-dense emit-value map-options v-model="fields[name+\'_poffset\']"' +
                ' :name="name+\'_poffset\'" :bg-color="fieldChanged(name+\'_poffset\')?\'blue-1\':inputBgColor"' +
                ' stack-label label="Offset" :options="dateOffsets" :form="form" behavior="menu"></q-select>' +
            '<q-select dense options-dense emit-value map-options v-model="fields[name+\'_period\']"' +
                ' :name="name+\'_period\'" :bg-color="fieldChanged(name+\'_period\')?\'blue-1\':inputBgColor"' +
...
Vue.component('m-drop-down', {
    ...
    template:
        ...
        //:bg-color="inputBgColor"
                ...
                ' stack-label :label="label" :loading="loading" :bg-color="bgColor" :options="curOptions">' + //placeholder="Enter char(s) to search"
            '<q-tooltip v-if="tooltip">{{tooltip}}</q-tooltip>' +
