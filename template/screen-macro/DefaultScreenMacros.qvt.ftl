    ...
    <#if listHasContent><#list listObject as listEntry>
        ...
        <#if !(isMulti || skipForm)>
            ...
            <m-form name="${formId}_${listEntry_index}" id="${formId}_${listEntry_index}" action="${formListUrlInfo.path}" v-slot:default="formProps"
                    :fields-initial="${Static["org.moqui.util.WebUtilities"].fieldValuesEncodeHtmlJsSafe(sri.makeFormListSingleMap(formListInfo, listEntry, formListUrlInfo, "first"))}">
        </#if>
        <#if isMulti>
            <#assign ownerForm = formId>
            <#assign fieldsJsName = "formProps.fields">
    ...
    <q-input dense bg-color="<@getInputBgColor/>" <#if fieldLabel?has_content> stack-label label="${fieldLabel}"</#if> name="${curFieldName}"<#rt>
        ...
        <template v-slot:after>
            <#if hideOptions != "operator">
                ...
                <q-select class="on-left" dense bg-color="<@getInputBgColor/>" options-dense emit-value map-options name="${curFieldName}_op"<#if ownerForm?has_content> form="${ownerForm}"</#if><#rt>
